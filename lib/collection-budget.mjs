...
export function reserveState(state,now=Date.now()) {
 if(!canCollect(state,now))throw Error('Hourly collection slot is unavailable');
 return {version:1,lastStartedAt:new Date(now).toISOString(),nextAllowedAt:new Date(now+HOUR_MS).toISOString()};
}
