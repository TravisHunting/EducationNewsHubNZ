...
export async function boundedFetch(raw,source,{robots=false,checkRedirect=null}={}) {
 let url=raw;
 for(let i=0;i<5;i++){
  if(!allowed(url,source))throw Error('Redirect or URL is outside the approved source hosts.');
  const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),12000);
  try{
   const r=await fetch(url,{redirect:'manual',signal:controller.signal,headers:{'User-Agent':USER_AGENT,Accept:robots?'text/plain':'text/html, application/rss+xml, application/xml;q=0.9'}});
   ...
   if(!robots&&!/(text\/html|xml|text\/plain)/i.test(r.headers.get('content-type')||'')){await r.body?.cancel();throw Error('Unexpected content type')}
   const limit=robots?512000:2000000;if(Number(r.headers.get('content-length'))>limit){await r.body?.cancel();throw Error('Response exceeds collection size limit')}
   ...
   const data=new Uint8Array(size);let offset=0;for(const c of chunks){data.set(c,offset);offset+=c.length}const text=new TextDecoder().decode(data);
...
export function extract(html,url,source,fallback){
 ...
 return {title:title.replace(/\s*[|]\s*(Ministry of Education|Education Review Office|NZQA).*$/i,'').slice(0,260),excerpt,publishedAt:parseDate(date||clean(specificDate)||textDate),documents:[...new Map(documents.map(d=>[d.url,d])).values()].slice(0,12)};
}
...
export async function collect(source,previous,limit=5){
 const now=new Date().toISOString();const prior=previous?.records||[];
 try{
  const robotsByOrigin=new Map();
  const check=async(url)=>{const u=new URL(url);if(!robotsByOrigin.has(u.origin)){const r=await boundedFetch(u.origin+'/robots.txt',source,{robots:true});robotsByOrigin.set(u.origin,r.text)}const rule=robotsDecision(robotsByOrigin.get(u.origin),u.pathname+u.search);if(!rule.allowed)throw Error('robots.txt disallows this collection path');if(rule.delay>20)throw Error('Publisher crawl delay exceeds this collector window');return rule.delay};
  const fetchPage=async(url)=>{const delay=await check(url);await new Promise(r=>setTimeout(r,delay*1000));return boundedFetch(url,source,{checkRedirect:check})};
  ...
  const known=new Set(prior.map(x=>x.url));const chosen=[...list.filter(x=>!known.has(x.url)),...list.filter(x=>known.has(x.url))].slice(0,limit);const incoming=[];const failures=[];
  for(const c of chosen){try{
   const detail=source.metadataOnly?{title:c.title,excerpt:'',publishedAt:null,documents:[]}:extract((await fetchPage(c.url)).text,c.url,source,c.title);
   ...
   incoming.push({id:await recordId(c.url),sourceId:source.id,url:c.url,...detail,topics,signal:/reform|announc|new curriculum|new qualification|funding|government|policy|budget/i.test(detail.title)?'Policy change':source.kind==='Research'?'Research':'Sector update',discoveredAt:now,retrievedAt:now,method:source.metadataOnly?'HTML index · headline only':'HTML index + publication metadata'});
